...
module.exports = {
	...
	list: function (cmd) {
		...
		client.get(url + "/admin:datasource_providers?pagesize=100&&sysorder=(name:asc_uc,name:desc)", {
						headers: {
						...
						printObject.printHeader('Datasource Providers');
						var table = new Table();
						var type = "";
						var verboseDisplay = "";
						_.each(data, function(p) {
							...
							if(cmd.verbose) {
							   verboseDisplay += "\n";
							   verboseDisplay += "lacadmin dataprovider export --provider_name '"+p.name+"' --file  DATAPROVIDER_"+p.name + ".json\n";
							   verboseDisplay += "#lacadmin dataprovider import --file DATAPROVIDER_"+p.name + ".json\n";
						   }
				});
			table.sort(['Name']);
			console.log(table.toString());
			printObject.printTrailer("# datasource_providers: " + data.length);
			if(cmd.verbose) {
	...
	del: function(cmd) {
		...
		client.get(loginInfo.url + "/admin:datasource_providers?sysfilter=" + filt, {
			headers: {
	...
	export: function(cmd) {
		...
		client.get(loginInfo.url + "/datasource_providers?pagesize=1000&"+filter, {
			headers: {
				Authorization: "CALiveAPICreator " + loginInfo.apiKey + ":1",
				"Content-Type" : "application/json"
			}
			...
			if (data.length === 0) {
				console.log(("No datasource provider found").red);
				return;
			}
	...
	import: function(cmd) {
		...
		var startTime = new Date();
		client.put(loginInfo.url + "/datasource_providers", {
			data: fileContent,
			headers: {
				Authorization: "CALiveAPICreator " + loginInfo.apiKey + ":1",
				"Content-Type" : "application/json"
			...
			printObject.printHeader('Datasource Provider created, including:');
			if(data.statusCode == 200 ){
				console.log("Request took: " + (endTime - startTime) + "ms");
				return;
			} 	
			var newProvider = _.find( data.txsummary, function(p) {
				return p['@metadata'].resource === 'admin:datasource_providers';
			});
			if ( ! newProvider) {
